...
from fairseq.modules import (
    AdaptiveSoftmax, LearnedPositionalEmbedding, EncoderAttention, DecoderAttention, SinusoidalPositionalEmbedding
)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        x = self.self_attn(x, x, encoder_padding_mask)
        x = self.dropout(x)
        x = residual + x
    ...
    def __init__(self, args, no_encoder_attn=False):
        ...
        if no_encoder_attn:
            ...
            self.encoder_attn = EncoderAttention(
                self.embed_dim, args.decoder_attention_heads,
                dropout=args.attention_dropout, static_kv=True
            )
