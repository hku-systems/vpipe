...
class EncoderAttention(MultiheadAttention):
    def __init__(self, embed_dim, num_heads, dropout=0., bias=False, static_kv=False):
        super(EncoderAttention, self).__init__(embed_dim, num_heads, dropout, bias)
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.dropout = dropout
        self.static_kv = static_kv

    def forward(self, query, key, key_padding_mask=None, incremental_state=None, need_weights=False):
        x, attn = super(EncoderAttention, self).forward(query, key, key, key_padding_mask=key_padding_mask,
                                                     incremental_state=incremental_state,
                                                     need_weights=need_weights, static_kv=self.static_kv)
        if attn is not None:
            return x, attn
        else:
            return x
    ...
    def __repr__(self):
        return 'EncoderAttention({}, {}, dropout={}, static_kv={})'.format(
            self.embed_dim, self.num_heads, self.dropout, self.static_kv
        )

class DecoderAttention(MultiheadAttention):
    def __init__(self, embed_dim, num_heads, dropout=0., bias=False):
        super(DecoderAttention, self).__init__(embed_dim, num_heads, dropout, bias)
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.dropout = dropout

    def forward(self, query, mask_future_timesteps=True,
                incremental_state=None, need_weights=False):
        x, _ = super(DecoderAttention, self).forward(query, query, query, mask_future_timesteps=mask_future_timesteps,
                                                     incremental_state=incremental_state,
                                                     need_weights=need_weights)
        return x

    def __repr__(self):
        return 'DecoderAttention({}, {}, dropout={})'.format(
